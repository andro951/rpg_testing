...
window.fetch=async(path,options={})=>{const r=await window.workerBridge(path,options);return new Response(Uint8Array.from(atob(r.body),c=>c.charCodeAt(0)),{status:r.status,headers:{'Content-Type':r.type}})};""")
                ...
                cards=page.locator('.model-card');first=cards.nth(0);second=cards.nth(1)
                first_select=first.locator('select');second_select=second.locator('select')
                expect(first_select).to_have_value('8');expect(second_select).to_have_value('8')
                expect(first).to_contain_text('VRAM: 8 GB assigned');expect(second).to_contain_text('VRAM: 8 GB assigned')
                first_save=first.get_by_role('button',name='Save VRAM',exact=True)
                expect(first_save).to_be_disabled()
                first_select.select_option('12')
                expect(first_save).to_be_enabled()
                expect(second_select).to_be_enabled();expect(second_select).to_have_value('8')
                first_save.click()
                expect(first).to_contain_text('VRAM: 12 GB assigned',timeout=5000)
                expect(first_select).to_have_value('12');expect(first_save).to_be_disabled()
                expect(second_select).to_be_enabled();expect(second_select).to_have_value('8')
                expect(page.locator('#state-badge')).to_have_text('IDLE')
                first.get_by_role('button',name='Accept recommended 8 GB',exact=True).click()
