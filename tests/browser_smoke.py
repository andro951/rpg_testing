...
window.fetch=async(path,options={})=>{const r=await window.localHttpTestBridge(path,options);return new Response(Uint8Array.from(atob(r.body),c=>c.charCodeAt(0)),{status:r.status,headers:{'Content-Type':r.type}})};''')
                ...
                expect(page.locator('#pending')).to_have_text('8')
                ...
                expect(page.locator('#pending')).to_have_text('0');expect(page.locator('#complete')).to_have_text('8')
                screenshot=os.environ.get('WORKBENCH_SCREENSHOT')
                if screenshot:page.screenshot(path=screenshot,full_page=True)
                page.locator('nav button[data-page="results"]').click();expect(page.locator('.result-row')).to_have_count(8)
                ...
                page.get_by_role('button',name='Delete / rerun',exact=True).first.click();expect(page.locator('.result-row')).to_have_count(7)
                ...
                page.locator('nav button[data-page="tests"]').click();expect(page.locator('#test-list article')).to_have_count(2)
                page.locator('#example-select').select_option('cached_questions');page.locator('#import-example').click()
                expect(page.locator('#test-list article')).to_have_count(3)
                page.locator('nav button[data-page="overview"]').click();page.locator('#run').click()
