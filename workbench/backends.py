    ...
    def request(self,path,body=None,stream=False,cancel=None):
        ...
        def partial():
            return {'text':''.join(text),'reasoning_text':''.join(reasoning),'raw_chunks':chunks,
                    'finish_reason':finish,'request_seconds':time.perf_counter()-started,
                    'usage':usage,'timings':timings,'incomplete':True}
        try:
            ...
            error=Cancelled('Cancelled') if cancel and cancel.is_set() else BackendError(str(exc))
            error.partial_response=partial();raise error from exc
        except (BackendError,Cancelled) as exc:
            exc.partial_response=partial();raise
        finally:
...
class DemoBackend:
    ...
    def load(self,model,context,cancel=None,layers="all",execution_class="full_gpu"):
        self.counter=0;self.load_metadata={'simulated':True,'context':context,'load_seconds':0.0,'health_exact_ready':True,'placement':{'status':'full_gpu','gpu_layers':33,'total_layers':33,'cpu_layers':0},'execution_class':execution_class}
...
from .native import NativeBackend as LocalBackend
