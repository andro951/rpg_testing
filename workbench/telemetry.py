...
def monitored_execute(test, variant, backend, cancel=None, on_stage=None, monitor=None, repetition=0, on_progress=None):
    from .workflows import execute
    if monitor: monitor.begin_case()
    try:
        result = execute(test, variant, backend, cancel, on_stage, repetition, on_progress)
    except Exception as exc:
        if monitor:
