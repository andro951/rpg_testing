...
1. **Choose your models folder.** It starts unset. On the GPU computer, **Choose models folder…** opens the operating system’s normal folder picker. The path is saved immediately and model scanning continues separately, so you can switch to **Installed models** while it scans. Existing GGUFs are discovered in place. A successful rescan/preflight treats this folder as the authoritative installed-model inventory: if you manually delete a model's GGUF files, its machine-local catalog assignment and stale artifact cache are removed automatically while historical benchmark results are retained. An unavailable/disconnected models folder is never treated as an empty authoritative scan. If empty, confirm **Use this empty folder**, or choose another. There is no automatic C-drive model location.
...
**Safety deadlines:** model loading has a 600-second (10-minute) infrastructure watchdog. Every benchmark test also has an explicit positive `timeout_seconds` field; all current tests and bundled examples are set to **60 seconds**. A case that reaches its test limit has only that active request cancelled, keeps its streamed partial output, is recorded as a terminal failed timeout, resets the prompt slot, and continues to the next case. Load-time stalls still stop the owned llama-server process. Hybrid layer trials are bounded to four; automatic context expansions to two. The policy is recorded with results.
